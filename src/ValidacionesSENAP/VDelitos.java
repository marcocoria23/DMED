...
" \n" +
" from RES_tr_senap_delitos deli left join\n" +
" RES_TR_SENAP_noticiacriminal nc \n" +
...
"from RES_tr_senap_delitos deli   \n" +
...
" from RES_tr_senap_delitos deli left join RES_tr_senap_delitos x on  x.DELITOID_RECLASIFICADO=deli.DELITOID\n" +
...
"SELECT COUNT(DELITOID)CUENTA,DELITOID FROM RES_TR_SENAP_DELITOS\n" +
...
"FROM RES_TR_SENAP_DELITOS WHERE DELITOLEYPENAL=0"
                + " and  (ENTIDADID='" + Entidad + "' AND PERIODO='" + Periodo + "')";
        System.out.println(sql);
        Query = sql;
        resul = conexion.consultar(sql);
...
public ArrayList normativo_LGMDFP(String Entidad, String Periodo) throws SQLException {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT ENTIDADID, DELITOID, CATFISCALIAID, CARPETAID, DELITOLEYPENAL, SENALAMIENTONORMATIVO, CATFUEROID FROM RES_TR_SENAP_DELITOS WHERE UPPER(SENALAMIENTONORMATIVO) LIKE '%LG%' AND UPPER(SENALAMIENTONORMATIVO) NOT LIKE '%ALG%' and CATFUEROID = 1";
        System.out.println(sql);
        Query = sql;
        resul = conexion.consultar(sql);
        try {
...
public ArrayList normativo_LGMDFP_dif(String Entidad, String Periodo) throws SQLException {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT ENTIDADID, DELITOID, CATFISCALIAID, CARPETAID, DELITOLEYPENAL, SENALAMIENTONORMATIVO, CATFUEROID FROM RES_TR_SENAP_DELITOS WHERE (UPPER(SENALAMIENTONORMATIVO) not LIKE '%LG%' AND UPPER(SENALAMIENTONORMATIVO) LIKE '%ALG%') or CATFUEROID not in (1,9) ";
        System.out.println(sql);
        Query = sql;
        resul = conexion.consultar(sql);
        try {
